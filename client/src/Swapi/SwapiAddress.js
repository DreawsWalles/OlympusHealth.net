...
//Получение данных с нашего сервера
...
//Получение данных с постороннего API
...
async function getRegionsByServer(country, region){
    let tmp;
    await fetch(`${Api}${Controllers["Address"]}Country?name=${country}`, {
        method:"Post",
        headers:{
...
export async function GetRegion(country, region){
    let serverData = getRegionsByServer(country, region);
}